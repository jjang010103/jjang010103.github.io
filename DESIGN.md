...
- 본문은 Pretendard, 계측값(날짜·번호·라벨)은 JetBrains Mono. 모노스페이스는 데이터에만 쓴다.
...
### Primary
- **잉크 (ink)**: 반전 블록의 바탕. 이름(`.hero-name`), 강조 칩(`.kw`, `strong`), 타임라인 세그먼트, 푸터, pill의 활성 챕터·시트의 켜진 언어. 라이트에선 거의 검정, 다크에선 밝은 회백색이라 "잉크"는 색이 아니라 **역할**이다.
...
- **Micro** (11.5~13.5px, 모노): pill 챕터 번호(11.5px, 0.06em), 타임라인 날짜(12.5px), 푸터 메타(12px), 카드 이미지 자리 캡션(11px).

### Named Rules
**The Mono Means Data Rule.** JetBrains Mono는 날짜, 번호, 라벨, 연락처 값에만 쓴다. 문장에는 쓰지 않는다. 수치 카드 라벨은 영어일 때만 모노고 한글·일본어는 본문 서체다.
...
- **경력**: 실제 근무 기간 비율의 타임라인 바 아래 회사별 행을 둔다. 각 행은 `회사·기간·소개 | 업무 불릿` 1:2 비율(gap 32px), 행 간격 28px. 기간 막대와 설명 열이 잘못 연결되지 않도록 회사 둘을 나란히 놓지 않는다.
- **스택**: 주력 기술을 상단 한 줄에 24px로 강조하고, 아래에 플랫폼·백엔드·도메인 기술을 3개 묶음으로 둔다. 표·행 구분선·칩·모노 서체는 쓰지 않는다. 보조 기술은 본문 서체 15px, 항목 간격 16px, 자연스러운 줄바꿈이다.
- **프로젝트 벤토**: 4열 그리드(gap 16px). 카드는 `PROJECTS[].span / minH / imgW`가 넘긴 값대로 2칸 또는 4칸, 최소 높이 170~200px. 이미지 폭은 데이터의 지정값과 카드 폭 30% 중 작은 값이다. 비어 있어도 단색 `card` 면과 캡션으로 자리를 유지한다. 모든 카드가 제목 → 개요 순서이며 성과는 상세 모달에만 둔다(ezZip만 `featured`).
- **중간 폭(721~1000px)**: 프로젝트를 한 행에 하나씩 놓아 본문 폭을 확보한다. 카드 내부의 좌우 배치는 유지한다.
- **모달**: 폭 `min(720px, 100vw - 32px)`, 높이 `min(90vh, 1100px)`, 페이지 위에 종이색 불투명 오버레이. 헤더 왼쪽에 페이저(‹ `n / 6` ›).

**모바일(≤720px)**은 시트 하단의 `@media (max-width: 720px)` 블록 하나에서만 재정의한다. 이름과 챕터 내비는 숨기고, 언어·테마·PDF 다운로드 아이콘을 상단 sticky 바에 둔다(44px 타깃, 그림자 없음). 바가 문서 흐름에 자리를 확보하며 앵커 이동 여백은 80px이다. 섹션 제목은 22px이고 긴 번역은 줄바꿈한다. 경력·프로젝트·학력은 1열, 카드 이미지는 하단 스트립(88 / 112px), 스택은 1열로 쌓고, 주력 기술은 21px로 표시한다. 카드에서는 수치와 성과 문구를 모두 숨기고 기존 intro로 프로젝트 개요를 표시한다. 모달은 92dvh 바텀시트(헤더 컨트롤 36px), 연락처 값은 링크 없는 텍스트이며 복사 버튼만 동작한다. GitHub·LinkedIn 링크는 유지한다.
...
### Named Rules
**The Floating-Only Shadow Rule.** 그림자는 열린 모달에만 접지 + 환경 두 겹으로 둔다. 상단 바·카드·버튼·칩은 호버에도 그림자를 얻지 않는다. 한 겹짜리 넓은 블러는 다크에서 얼룩으로 보이므로 쓰지 않는다.
...
테두리는 1px 헤어라인이다. 프로젝트 카드·pill은 `hairline`, 시트·수치 카드는 `hairline-soft`, 컨트롤은 `control-border`. 잉크 반전 블록은 테두리가 없다. 빗금은 타임라인 공백에만 남긴다. 빈 카드 이미지는 단색 면으로 조용하게 유지한다.

## Components

### Buttons
- **pill 컨트롤** (상단 바의 테마·PDF): 28px, 테두리·바탕 없음. 둘 다 16px SVG이며 PDF는 아래 화살표와 받침 모양의 다운로드 아이콘이다. PDF 버튼은 기존 인쇄/PDF 저장 대화상자를 열고 3개 국어 `aria-label`·`title`을 유지한다. 모바일 타깃은 44px.
...
- **기술 스택**: 칩을 쓰지 않는다. 주력 기술은 24px(모바일 21px), 나머지는 15px 본문 서체의 텍스트다. 분류 제목은 15px·600 웨이트다.
- **모달 태그** (`.tag`): 14px, 패딩 5px 10px, 컨트롤 테두리 1px, 9px 모서리, 글자 `text`.

### Cards / Containers
- **프로젝트 카드(벤토)**: 바탕 `paper`, 헤어라인 1px, 모서리 14px. 회사·기간 → 제목 → 기존 `intro` 개요만 표시한다. 성과 수치·성과 문구는 상세 모달에만 남긴다. 이미지는 카드 폭의 최대 30%, 없으면 단색 면과 11px 모노 캡션이다. 모바일에서는 이미지가 아래로 내려가며 누름 효과는 `scale(.986)`이다.
- **수치 카드(모달)**: 바탕 `card-item`, 14px 스쿼클, 패딩 18/20/16, 값 30px 600 위에 라벨. 그리드 `repeat(auto-fit, minmax(160px, 1fr))`이라 2장·3장 모두 빈 칸 없이 찬다.
- **모달 시트**: 720px, 24px 스쿼클, 바탕 `card`, 두 겹 그림자, 패딩 22/44/40. 헤더에 페이저(테두리 없는 ‹ › + `n / 6`, 화살표 라벨에 이전·다음 프로젝트 이름) · 언어 세그먼트 · 테마 · 닫기. 좌우 화살표 키로도 넘긴다. 모바일은 바텀시트(위 모서리만 24px, 드래그 핸들, 아래로 90px 스와이프하면 닫힘). 본문 위에 페이드를 덮지 않아 마지막 줄까지 읽힌다.
...
- **모바일**: 챕터 칸과 이름을 숨기고 컨트롤 pill을 상단 sticky 바 오른쪽에 둔다(44px 타깃). 위치 표시는 브라우저 스크롤바가 맡는다.
...
- **Don't** 카드의 이미지 칸을 본문으로 메우지 않는다. `img`가 올 때까지 단색 면과 캡션으로 남긴다.
