...
페이지 최대 폭 1344px(1440 뷰포트 기준 좌우 48px), 위 여백 32px. 모든 형제 간격은 `gap`으로 잡는다.
...
- **01 프로필 (벤토)**: 4열 × 3행.
  ...
  - 기술 스택: 아랫줄 4열 전체 폭, 흰 카드. `STACK`의 네 묶음을 4열로 둔다. 핵심 스택(첫 묶음)만 `surface-inset` 알약으로 강조하고, 나머지 묶음은 알약 없는 15px 글자(`body`)로 흘린다.
...
- **03 프로젝트**: 4열.
  ...
  - 카드 순서는 `CARD_ORDER`가 정한다(ezZip · 아이센드 · PC3D / Ez-Spool · 보운 ERP · MakeTApp). 왼쪽 두 칸 열에 ezZip(2×2)과 Ez-Spool이 위아래로, 오른쪽에 가로형 아이센드·PC3D, 맨 아래 줄 오른쪽에 1칸 보운 ERP·MakeTApp이 나란히 놓인다(사용자 결정, 2026-09-25). 상세 페이지의 번호와 이전·다음은 `PROJECTS` 순서를 따른다.
...
**프로젝트 상세 페이지**는 메인 콘텐츠를 대체한다(내비 바는 그대로). 순서: "← 메인으로" + 페이저 줄 → 히어로 벤토 → 대표 이미지 → 수치 카드 줄 → 본문 카드(01~05) → 메인과 같은 푸터. 세부는 Components의 Project Detail Page에 있다.

**중간 폭(721~1000px)**: 그리드를 2열로 줄인다. 메인·상세 히어로는 2열 × 1행, 스택은 2열(묶음도 2열), 대표 프로젝트는 2열 × 1행, 상세 본문 카드는 모두 2열. 내비의 이름은 숨기고 챕터가 왼쪽으로 붙는다.

**모바일(≤720px)**은 시트 하단의 `@media (max-width: 720px)` 블록 한 곳에서만 재정의한다. 벤토는 **반반 섞기**다(사용자 결정, 2026-09-26): 2열 그리드에서 글이 긴 카드(히어로·기술 스택·2칸 프로젝트·학교·병역·상세 히어로·결과물)는 전체 폭, 짧은 카드(총 경력·연락처·1칸 프로젝트·자격증·상세 기간·기술 스택)만 반씩 나란히 놓는다. 경력과 수치 카드는 1열. 프로젝트는 강조(잉크) 카드 ezZip·Ez-Spool을 맨 위로, 흰 카드를 그 아래로 모은다(CSS `order`, 데스크톱의 왼쪽 열과 같은 순서). 글자 크기는 줄이지 않는다. 반 칸 연락처는 값 대신 메일·전화 아이콘 버튼(누르면 복사)과 두 줄 링크다. 페이지 좌우 16px, 카드 gap 12px, 카드 모서리 22px, 카드 패딩 24px. Display 44px, Headline 32px, Stat 64px. 히어로 카드는 모바일에서도 안쪽 여백 40px 32px, 한 줄 소개 20px, 자기소개 15px. 한 줄 소개의 두 번째 문장(인라이플·ezZip)은 숨기고(`.lead-more`), 그 자리(390px 기준 한·영 3줄, 일 2줄)를 이름 아래 간격(20px + 3lh / 일본어 2lh)으로 돌려 카드 크기는 원래대로 둔 채 글을 아래로 내린다. 상단 바는 없고, 내비는 **화면 아래에 떠 있는 알약**이다(오른쪽 아래, 내용 폭만큼, 알약 전체 `zoom: .9` — 높이 약 47px, 안쪽 여백 6px + 버튼 40px로 끝부분 링을 고르게, 실제 버튼 타깃 약 36px): 언어 3 · 테마 · 메뉴 버튼. 메뉴를 누르면 **알약 자체가 위로 늘어나** 카드(모서리 26px)가 되고, 버튼 줄 위에 18px 챕터 목록(48px 행, 켜진 챕터 모서리 20px = 카드 26 − 여백 6)과 PDF 저장(구분선 없이 같은 목록)이 펼쳐진다. 뒤를 어둡게 하지 않고, 바 뒤 점진 흐림이 190px에서 520px로 늘어나 메뉴 카드 위까지 덮는다(250ms). 바깥을 누르면 닫힌다. 아래로 스크롤하는 동안 알약은 화면 아래로 빠지고(흐림도 함께 꺼짐, 280ms), 위로 스크롤하면 돌아온다. 맨 위·맨 끝·메뉴가 열려 있을 때·키보드 포커스가 안에 있을 때는 항상 보인다. 딤·챕터·Esc로 닫힌다. 이름은 히어로 카드가 보여 준다. 바 뒤 점진 흐림도 아래로 뒤집혀(높이 190px), 알약 뒤에 내용이 겹칠 때만 켜지고 맨 끝까지 내리면 꺼져 알약이 쟁반 위에서 플랫해진다(페이지 아래 여백 104px, 알약은 아래에서 30px). 사용자 결정, 2026-09-26 — 두 줄 상단 바·상단 한 줄 메뉴 대체. 가로형 프로젝트의 이미지는 설명 아래로 내린다. 푸터는 한 열로 쌓고 왼쪽 정렬한다.
...
- **상단 줄** (`data-noprint`): 왼쪽 "← 메인으로" 흰 알약(44px), 오른쪽 이전 · `01 / 06` · 다음(44px 흰 원). 화살표 라벨에는 이전·다음 프로젝트 이름이 붙는다. 페이저와 좌우 화살표 키는 끝에서 멈춘다.
- **히어로 벤토** (4열): 액센트 카드 2열 × 2행(`PROJECT nn · 회사`, Display Detail 제목, blurb). 오른쪽 위에 정보 카드(기간, 인원·형태는 데이터에 있을 때만)와 기술 스택 카드가 1열씩. 기술 스택은 알약 없이 Space Grotesk 17px 500 글자를 기간처럼 카드 바닥에 붙인다(사용자 결정, 2026-09-25). 오른쪽 아래에 잉크 카드 "결과물 보기" + 링크 알약(줄 높이 160px로 낮게, 남는 높이는 위 줄이 받는다 — 사용자 결정, 2026-09-25) — 링크가 없어도 잉크 칸은 내용 없이 남긴다(정보·스택 카드를 길게 늘이면 어색했다 — 사용자 결정, 2026-09-25). 인쇄에서는 빈 칸을 뺀다.
...
- 상세 뷰에서 챕터를 누르면 메인으로 돌아가 그 섹션으로 간다.
