...
거부한 것: 그라디언트 워시, 네온, 글래스모피즘, 카드마다 아이콘을 얹은 기능 소개 그리드, 이모지, 왼쪽 컬러 테두리 카드, 떠 있는 모달.
...
- **헤어라인 (hairline)**: 비활성 아이콘 글리프, 인쇄에서 흰 카드의 테두리, 스크롤 중 상단 바의 테두리에만 쓴다. 화면의 카드는 테두리 없이 톤 차이로 구분한다.
...
- **내비 바**: sticky(`top: 16px`) 흰 알약 하나. 맨 위에서는 테두리가 없고, 스크롤하면(4px 이상) `hairline` 한 줄(`box-shadow: inset 0 0 0 1px`)이 바 가장자리에 생겨 흰 카드와 구분된다(사용자 결정, 2026-09-25 — 띠·테·그림자·반전·숨김은 기각). 왼쪽에 이름만(900, 20px) 둔다. `.dev`나 액센트 점은 붙이지 않는다. 오른쪽에 챕터 4개(프로필 · 경력 · 프로젝트 · 학력·자격증)와 언어·테마·PDF 아이콘 버튼. 켜진 챕터는 잉크 알약이다. 메인에서는 바 아랫변 기준으로 스크롤에 따라, 상세에서는 항상 "프로젝트"가 켜진다.
...
- **03 프로젝트**: 4열.
  ...
  - 칸 수는 `PROJECTS[].span / rows`가 정하고 CSS는 `--span / --rows`만 읽는다. 대표(2×2) 옆 두 번째 줄은 내용 높이만큼만 쓰고(`grid-template-rows: 1fr auto`), 대표가 더 필요한 높이는 첫 줄이 받는다(PC3D를 낮게 — 사용자 결정, 2026-09-25).
...
**모바일(≤720px)**은 시트 하단의 `@media (max-width: 720px)` 블록 한 곳에서만 재정의한다. 1열, 페이지 좌우 16px, 카드 gap 12px, 카드 모서리 22px, 카드 패딩 24px. Display 44px, Headline 32px, Stat 64px. 내비는 이름을 숨기고 두 줄이다: 위에 언어·테마·PDF, 아래에 가로로 스크롤되는 챕터 알약(터치 타깃 44px). 가로형 프로젝트의 이미지는 설명 아래로 내린다. 푸터는 한 열로 쌓고 왼쪽 정렬한다.
...
평면이 기본이다. 깊이는 **쟁반 → 카드 → 카드 위 면** 세 단의 톤 차이로만 표현한다. 화면의 카드에는 테두리도 그림자도 없다(인쇄의 헤어라인, 스크롤 중 상단 바의 헤어라인은 예외). 그림자 토큰은 없다.

### Named Rules
**The Flat Tray Rule.** 카드·내비·버튼·칩은 호버해도 그림자를 얻지 않는다. 스크롤 중의 상단 바도 그림자 대신 헤어라인 한 줄이다. 프로젝트 카드의 호버는 제목 밑줄이다(카드를 `surface-inset`으로 가라앉히면 같은 색의 태그가 사라진다). 내비 항목·아이콘 버튼·"메인으로"·페이저는 `surface-inset` 바탕으로 호버한다.
